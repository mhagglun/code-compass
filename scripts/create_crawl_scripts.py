    ...
    for i, gitgrab in enumerate(gitgrabs):
        ...
        if ds != lastds:
            ...
            if gitclone:
                ...
                downfd.write(
                    '\tgit clone git://github.com/$projdir/$projname.git &>> $currdir/dataset$ds/git-download.log\n')
                downfd.write(
                    '\ttar -czf $currdir/dataset$ds/$projdir/$projname.tar.gz $projname/\n')
                ...
                downfd.write(
                    '\twhile [ ! -f $fname -a ! -f $fname.error ] ; do\n')
                downfd.write(
                    '\t\tcurl -L "https://api.github.com/repos/$projdir/$projname/tarball?access_token=$APIKEY" 2>/dev/null > $fname\n')
                downfd.write('\t\tif ! gzip -t $fname 2>/dev/null ; then\n')
                downfd.write(
                    '\t\t\tif grep -q "API Rate Limit Exceeded" $fname ; then\n')
                ...
                downfd.write(
                    '\t\t\t\techo "Skipping project $projdir/$projname"\n')
                downfd.write('\t\t\t\tbreak\n')
                downfd.write('\t\t\tfi\n')
                downfd.write('\t\tfi\n')
                downfd.write('\t\tsleep 1\n')
